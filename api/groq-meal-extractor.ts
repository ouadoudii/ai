...
const MEAL_SCHEMA = {
  ...
  schema: { type: 'object', additionalProperties: false, properties: {
    mealDetected: { type: 'boolean' }, mealTitle: { type: 'string' }, mealItems: { type: 'array', items: { type: 'string' } }, mealCategory: { type: 'string' }, mealContext: { type: 'string' },
...
Understand the COMPLETE message before extracting anything. The user may speak Arabic/Darija, German, French, English, mixed language or transliteration.
${languageInstruction}
For meals: create one meals[] entry PER distinct meal moment. Preserve quantities, preparation and ingredients. For variable or composite meals, semantically separate the dish from components already stated. Identify only nutritionally relevant missing details that could materially change the interpretation (for example main protein/alternative, serving format or starch/base, sauce/dressing, substantial extras, or portion size). Never ask again for details already stated. If relevant details remain uncertain, mealContext must contain one short natural follow-up question in the user's language that groups only those missing details; do not turn it into a form. If nothing material is missing, mealContext stays empty. Do not use dish-specific keyword rules: reason from the composition and variability of any real dish, including unfamiliar dishes. Do not invent components.
...
Open vocabulary: recognize arbitrary real dishes and drinks. Exclude negated, hypothetical, planned or merely desired items. Repair obvious ASR variants conservatively. If uncertain, omit rather than guess. Legacy meal fields summarize the first meal for compatibility.`;
