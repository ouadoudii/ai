...
export type AppLanguage = 'en' | 'ar' | 'de';
...
const dictionary: Dictionary = {
  ...
  chooseLanguage:{en:'Choose language',ar:'اختر اللغة'}, close:{en:'Close',ar:'إغلاق'}
};
const de:Record<string,string>={today:'Heute',add:'Moment hinzufügen',patterns:'Entdeckungen',sleepFoodEnergy:'Schlaf · Essen · Energie',mainNav:'Hauptnavigation',nothingLogged:'Ein neuer Tag',entry:'Moment',entries:'Momente',todaySuffix:'heute',captureOnly:'Starte mit einem kleinen Moment.',dayBuilds:'Dein Bild wird mit jedem Moment klarer.',captureHint:'Essen, Schlaf, Energie oder Stimmung — ein Foto, deine Stimme oder ein paar Klicks genügen.',todaySoFar:'Deine Momente heute',tooEarly:'Wir lernen deinen Rhythmus kennen',whatRepeats:'Was entdeckst du über dich?',everydayReveal:'Ein paar Alltagsmomente können überraschend hilfreiche Zusammenhänge zeigen.',viewPatterns:'Meine Entdeckungen ansehen',noPerfectDay:'Kein perfekter Tag nötig — das echte Leben zählt.',noticeDontObsess:'Neugier statt Perfektion',understandRhythm:'Entdecke, was dir hilft, dich am besten zu fühlen.',onboardingBody:'Teile ein paar Alltagsmomente zu Essen, Schlaf, Energie und Stimmung. Mit der Zeit verbinden wir die Punkte und zeigen deinen persönlichen Rhythmus.',photo:'Foto',speak:'Erzähl mir',tap:'Schnellcheck',noPerfectTracking:'Mach es dir leicht.',ordinaryEnough:'Du musst nicht alles festhalten. Kleine, ehrliche Momente reichen, um daraus zu lernen.',startToday:'Meinen Rhythmus entdecken',language:'Sprache',addWhat:'Was möchtest du festhalten?',addLabel:'Ein kurzer Moment',again:'Nochmal dasselbe?',type:'Schreiben',captureKinds:'Wähle, was sich am einfachsten anfühlt. Ein paar Sekunden genügen.',yourData:'Deine Entdeckungen',patternsIntro:'Hier werden Alltagsmomente nach und nach zu hilfreichen Entdeckungen über dich.',observations:'Momente',tryWeek:'Ein kleines Experiment',realDataPoints:'Momente, die uns beim Lernen helfen',ordinaryDaysEnough:'Der Alltag gibt uns die besten Hinweise.',chooseLanguage:'Sprache wählen',close:'Schließen'};
...
export const LanguageProvider:React.FC<React.PropsWithChildren>=({children})=>{const [language,setLanguageState]=React.useState<AppLanguage>(()=>{try{const saved=localStorage.getItem('rhythm_language_v1');const locales=typeof navigator!=='undefined'?(navigator.languages?.length?[...navigator.languages]:[navigator.language]):[];return resolveInitialLanguage(saved,locales)}catch{return'en'}});const setLanguage=React.useCallback((lang:AppLanguage)=>{setLanguageState(lang);try{localStorage.setItem('rhythm_language_v1',lang)}catch{}},[]);React.useEffect(()=>{document.documentElement.lang=language;document.documentElement.dir=language==='ar'?'rtl':'ltr';document.body.dir=language==='ar'?'rtl':'ltr'},[language]);const t=React.useCallback((key:string)=>language==='de'?(de[key]??dictionary[key]?.en??key):(dictionary[key]?.[language]??key),[language]);return <LanguageContext.Provider value={{language,setLanguage,t}}>{children}</LanguageContext.Provider>};
