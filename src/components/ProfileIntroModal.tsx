...
interface Props{isOpen:boolean;initialTranscript?:string;onClose:()=>void;onSaved:()=>void;}
...
export const ProfileIntroModal:React.FC<Props>=({isOpen,initialTranscript='',onClose,onSaved})=>{
  ...
  return <div data-testid="profile-intro-modal" className="fixed inset-0 z-[95] flex items-end justify-center bg-[#211F1B]/70 p-0 backdrop-blur-lg sm:items-center sm:p-5" role="dialog" aria-modal="true">
    <section className="max-h-[92vh] w-full overflow-y-auto rounded-t-[32px] bg-[#FCFAF6] p-5 shadow-2xl sm:max-w-lg sm:rounded-[32px] sm:p-7" dir={language==='ar'?'rtl':'ltr'}>
      ...
      {draft&&!loading&&<>
        ...
        <label className="mt-6 block text-sm font-black text-[#3D403A]">{copy.summary}<textarea data-testid="profile-summary" value={draft.summary} onChange={e=>setDraft({...draft,summary:e.target.value})} rows={4} className="mt-2 w-full resize-none rounded-2xl border border-[#DDD5C9] bg-white px-4 py-3 font-medium outline-none focus:border-[#293D34]"/></label>
        <label className="mt-4 block text-sm font-black text-[#3D403A]">{copy.priorities}<textarea data-testid="profile-priorities" value={draft.priorities.join('\n')} onChange={e=>updateList('priorities',e.target.value)} rows={3} className="mt-2 w-full resize-none rounded-2xl border border-[#DDD5C9] bg-white px-4 py-3 font-medium outline-none focus:border-[#293D34]"/></label>
        <label className="mt-4 block text-sm font-black text-[#3D403A]">{copy.preferences}<textarea data-testid="profile-preferences" value={draft.preferences.join('\n')} onChange={e=>updateList('preferences',e.target.value)} rows={3} className="mt-2 w-full resize-none rounded-2xl border border-[#DDD5C9] bg-white px-4 py-3 font-medium outline-none focus:border-[#293D34]"/></label>
        <section data-testid="personal-first-plan" className="mt-5 rounded-[26px] bg-[#EEF1E8] p-5 text-[#293D34]">
          <div className="text-[11px] font-black uppercase tracking-[0.18em] text-[#667463]">{copy.plan}</div><h3 className="mt-2 text-xl font-black leading-tight">{draft.firstPlan.title}</h3>
          ...
          <div className="mt-4 rounded-2xl bg-white p-4"><div className="text-xs font-black text-[#7A8177]">{copy.firstStep}</div><p data-testid="personal-first-step" className="mt-1 text-sm font-black leading-6 text-[#343A32]">{draft.firstPlan.firstStep}</p></div>
        </section>
