...
export const AddMomentModal:React.FC<AddMomentModalProps>=({isOpen,onClose,onSave,editingMoment,initialCategory,completedMealCategories=new Set()})=>{
  ...
  return <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/45 backdrop-blur-sm sm:p-4"><div className="w-full sm:max-w-lg max-h-[92vh] overflow-y-auto rounded-t-[34px] sm:rounded-[34px] bg-[#F7F5F0] shadow-2xl">
    ...
    <div className="px-5 pb-6">
      ...
      <div className="mt-2 flex items-center gap-1.5 text-[10px] text-[#918D84]"><Globe2 className="w-3.5 h-3.5"/>{items.length?(ar?`تم اختيار ${items.length} من أطباق هذه الوجبة — أضف طبقاً آخر إن وجد`:`${items.length} dish${items.length===1?'':'es'} selected — add another if needed`):country?(ar?'نقترح أولاً ما يناسب منطقتك':'We prioritize what fits your region'):(ar?'اكتب فقط، وسنكمل معك':'Just type — we’ll help complete it')}</div>
