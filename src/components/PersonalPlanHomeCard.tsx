...
import { ArrowRight, CheckCircle2, Compass, Sparkles, Activity } from 'lucide-react';
...
export const PersonalPlanHomeCard:React.FC<Props>=({onStart})=>{
  ...
  const copy=language==='de'
    ?{eyebrow:'Heute wichtig',why:'Was deine Daten gerade zeigen',next:'Eine kleine Sache für heute',phase:{morning:'Morgen-Check-in starten',midday:'Mittag-Check-in starten',evening:'Abend-Check-in starten'}}
    :language==='fr'
      ?{eyebrow:'Important aujourd’hui',why:'Ce que montrent tes données',next:'Une petite action pour aujourd’hui',phase:{morning:'Commencer le check-in du matin',midday:'Commencer le check-in de midi',evening:'Commencer le check-in du soir'}}
      :language==='ar'
        ?{eyebrow:'المهم اليوم',why:'ما الذي تظهره بياناتك الآن',next:'خطوة صغيرة لليوم',phase:{morning:'ابدأ تسجيل الصباح',midday:'ابدأ تسجيل منتصف اليوم',evening:'ابدأ تسجيل المساء'}}
        :{eyebrow:'What matters today',why:'What your data is showing',next:'One small thing for today',phase:{morning:'Start morning check-in',midday:'Start midday check-in',evening:'Start evening check-in'}};
