...
export const MealVisualPicker:React.FC<MealVisualPickerProps>=({value,onChange,timePhase})=>{
  ...
  React.useEffect(()=>{setCountry(null);fetch('/api/locale',{cache:'no-store'}).then(r=>r.ok?r.json():null).then(d=>{setCountry(d?.country?String(d.country).toUpperCase():null)}).catch(()=>setCountry(null))},[]);
  ...
  return <div>
    ...
    <div className="mt-2 flex items-center gap-1.5 text-[10px] text-stone-500"><Globe2 className="w-3.5 h-3.5"/>{country?(ar?`اقتراحات محلية لـ ${country} أولاً`:`Local suggestions for ${country} first`):(ar?'اقتراحات عامة حتى يتم تحديد بلدك':'General suggestions until your country is detected')}</div>
