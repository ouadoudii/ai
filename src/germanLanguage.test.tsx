...
describe('German language',()=>{
 ...
 it('renders the discoveries view in German without crashing',()=>{let html='';expect(()=>{html=renderToString(<LanguageProvider><NutritionTypeAnalysisView moments={[]} checkIns={[]} onOpenCheckIn={noop} onOpenAddMoment={noop}/></LanguageProvider>)}).not.toThrow();expect(html).toContain('Entdeckungen');expect(html).toContain('Fuchs');expect(html).not.toContain('Fox · Feelings & Food');});
