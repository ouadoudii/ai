...
export default function App(){
  ...
  const openFoodCapture=React.useCallback((category:import('./types').MomentCategory|null=null,fromCapture=false,initialText='')=>{mealStartedAt.current=Date.now();setInitialMealCategory(category);setInitialMealText(initialText);setAddModalHasBack(fromCapture);trackUx({eventName:'flow_started',surface:'meal_editor',language,metadata:{source:fromCapture?'capture_choice':'direct'}});setEditingMoment(null);setIsAddModalOpen(true);},[language]);
  ...
  const backFromMeal=()=>{setIsAddModalOpen(false);setInitialMealText('');setInitialMealCategory(null);setAddModalHasBack(false);setIsCaptureOpen(true)};
