...
export default function App() {
  ...
  // Ask automatically only when today's check-in has not already been completed.
  // The completion check is based on saved data, so it survives page reloads and new sessions.
  React.useEffect(() => {
  ...
  return <div className="min-h-screen bg-[#FAFAF9] text-stone-900 pb-24 md:pb-0"><Header activeTab={activeTab} setActiveTab={setActiveTab} filterState={filterState} setFilterState={setFilterState} onOpenAddModal={() => { setEditingMoment(null); setIsAddModalOpen(true); }} onOpenCheckInModal={() => setIsCheckInModalOpen(true)} isMobilePreviewMode={isMobilePreviewMode} setIsMobilePreviewMode={setIsMobilePreviewMode} /><main className={`mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6 ${isMobilePreviewMode ? 'max-w-md' : 'max-w-7xl'}`}>{renderMainContent()}</main><MobileBottomNav activeTab={activeTab} setActiveTab={setActiveTab} onOpenCheckInModal={() => setIsCheckInModalOpen(true)} /><AddMomentModal isOpen={isAddModalOpen} onClose={() => { setIsAddModalOpen(false); setEditingMoment(null); }} onSave={handleSaveMoment} editingMoment={editingMoment} momentLabels={MOMENT_LABELS} /><DailyCheckInModal isOpen={isCheckInModalOpen} onClose={() => setIsCheckInModalOpen(false)} onSave={handleSaveCheckIn} recentMoments={moments} /><MomentDetailModal moment={selectedMomentDetail} isOpen={!!selectedMomentDetail} onClose={() => setSelectedMomentDetail(null)} onEdit={(m) => { setSelectedMomentDetail(null); setEditingMoment(m); setIsAddModalOpen(true); }} onDelete={handleDeleteMoment} onToggleFavorite={handleToggleFavorite} /></div>;
}
