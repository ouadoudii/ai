...
import { extractFoodPhraseFromVoice } from './utils/arabicFoodIntelligence';
...
export default function App(){
  ...
  const openFoodCapture=React.useCallback((category:import('./types').MomentCategory|null=null,fromCapture=false,initialText='',initialItems:string[]=[],initialNotes='')=>{mealStartedAt.current=Date.now();setInitialMealCategory(category);setInitialMealText(initialText);setInitialMealItems(initialItems);setInitialMealNotes(initialNotes);setAddModalHasBack(fromCapture);trackUx({eventName:'flow_started',surface:'meal_editor',language,metadata:{source:fromCapture?'capture_choice':'direct'}});setEditingMoment(null);setIsAddModalOpen(true);},[language]);
  ...
  const cancelMeal=()=>{setInitialMealCategory(null);setInitialMealText('');setInitialMealItems([]);setInitialMealNotes('');setAddModalHasBack(false);if(mealStartedAt.current){trackUx({eventName:'flow_finished',surface:'meal_editor',language,durationMs:Date.now()-mealStartedAt.current,outcome:'cancelled'});mealStartedAt.current=null;}setIsAddModalOpen(false);setEditingMoment(null)};
  const backFromMeal=()=>{setIsAddModalOpen(false);setInitialMealText('');setInitialMealItems([]);setInitialMealNotes('');setInitialMealCategory(null);setAddModalHasBack(false);setIsCaptureOpen(true)};
