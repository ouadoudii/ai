...
type Moment = { id:number; title:string; meal:string; note:string; favorite:boolean; rating:number; date:string };
const seed: Moment[] = [
...
export default function App(){
 ...
 const add=()=>{if(!title.trim())return;setMoments([{id:Date.now(),title:title.trim(),meal:'Moment',note:'A new food memory.',favorite:false,rating:5,date:'Just now'},...moments]);setTitle('');setAdding(false)};
 return <div className="app-shell">
  ...
  <main>
   ...
   {tab==='Stats' && <section className="feature"><BarChart3 size={30}/><h2>Your patterns</h2><div className="stats"><div><b>{moments.length}</b><span>moments</span></div><div><b>{favs.length}</b><span>favorites</span></div><div><b>{moments.length?Math.round(moments.reduce((a,m)=>a+m.rating,0)/moments.length):0}/5</b><span>avg. joy</span></div></div></section>}
   ...
   {(tab==='Today'||tab==='Favorites') && <section className="cards">{shown.map(m=><article className="moment" key={m.id}><div className="food-art"><span>{m.meal}</span></div><div className="moment-body"><div className="moment-title"><div><small>{m.date}</small><h4>{m.title}</h4></div><button onClick={()=>setMoments(moments.map(x=>x.id===m.id?{...x,favorite:!x.favorite}:x))}><Heart fill={m.favorite?'currentColor':'none'} size={21}/></button></div><p>{m.note}</p><div className="rating">{'★'.repeat(m.rating)}<span>{m.rating}.0</span></div></div></article>)}</section>}
  </main>
