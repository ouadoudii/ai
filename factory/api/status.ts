...
import { ensurePullRequestMerged, getQualityGateState } from '../server/lib/github';
import { ensureVercelProject, getProductionDeployment } from '../server/lib/vercel';
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (gate.state === 'pending') {
      return res.status(200).json({ ...job, stage: 'testing', message: gate.message, actionsUrl: gate.actionsUrl });
    }
    if (gate.state === 'failure') {
      return res.status(200).json({ ...job, stage: 'blocked', message: gate.message, actionsUrl: gate.actionsUrl });
    }
