...
async function authenticated(page: import('@playwright/test').Page) {
  await page.route('**/api/session', async (route) => {
    if (route.request().method() === 'GET') return route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ authenticated: true }) });
    return route.fallback();
  });
}

test('factory is protected before the idea form is exposed', async ({ page }) => {
...
test('authenticated intake explains the strict factory contract', async ({ page }) => {
...
test('a green remote pipeline becomes a live app without a second user action', async ({ page }) => {
  await authenticated(page);
  ...
  await page.route('**/api/status', (route) => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ appName: 'Quote Pilot', repoName: 'quote-pilot', repo, headSha, productionSha, prNumber: 7, stage: 'ready', message: 'Alle Gates sind grün.', githubUrl: 'https://github.com/ouadoudii/quote-pilot/pull/7', actionsUrl: 'https://github.com/ouadoudii/quote-pilot/actions/runs/1', vercelUrl: 'https://quote-pilot.vercel.app' }),
  }));
