...
export async function createRepository(name: string, description: string) {
  ...
  const profileResult = await gh('/user') as { data: { login: string } };
  const route = profileResult.data.login.toLowerCase() === owner.toLowerCase() ? '/user/repos' : `/orgs/${encodeURIComponent(owner)}/repos`;
  const result = await gh(route, {
    method: 'POST',
    body: JSON.stringify({ name, description, private: isPrivate, auto_init: true }),
  }) as { data: { full_name: string; html_url: string; default_branch: string } };
  return result.data;
}

export async function createBranch(repo: string, baseBranch: string, branch: string) {
  const base = await gh(`/repos/${repo}/git/ref/heads/${encodeURIComponent(baseBranch)}`) as { data: { object: { sha: string } } };
...
export async function getQualityGateState(repo: string, sha: string) {
  const result = await gh(`/repos/${repo}/actions/runs?head_sha=${encodeURIComponent(sha)}&event=push&per_page=50`) as {
  ...
  const runs = result.data.workflow_runs
    .filter((run) => run.name === 'Factory Quality Gate' && run.head_sha === sha)
    .sort((a, b) => b.id - a.id);
  const run = runs[0];
  ...
  if (run.conclusion !== 'success') return { state: 'failure' as const, actionsUrl: run.html_url, message: `Factory Quality Gate ist ${run.conclusion ?? 'fehlgeschlagen'}. Es wird nichts gemerged oder deployed.` };
...
export async function ensurePullRequestMerged(repo: string, prNumber: number, headSha: string) {
  ...
  if (current.data.merged) {
    if (!current.data.merge_commit_sha) throw new Error('Pull request is merged but GitHub returned no merge commit SHA.');
  ...
  if (!merged.data.merged) throw new Error(`GitHub refused to merge the green bootstrap PR: ${merged.data.message}`);
