...
export async function ensureVercelProject(name: string, repoFullName: string) {
  ...
  if (lookup.status !== 404) throw new Error(`Vercel project lookup failed (${lookup.status}): ${(await lookup.text()).slice(0, 300)}`);
...
export async function getProductionDeployment(projectId: string, sha: string) {
  ...
  if (!response.ok) throw new Error(`Vercel deployment lookup failed (${response.status}): ${(await response.text()).slice(0, 300)}`);
